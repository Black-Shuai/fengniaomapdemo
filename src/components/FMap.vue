  ...
  methods: {
    openMap () {
      ...
      this.map.on('mapClickNode', (event) => {
        ...
        console.log('aa', event, event.eventInfo.coord.x, event.eventInfo.coord.y)
        aa.push({'x': event.eventInfo.coord.x, 'y': event.eventInfo.coord.y, 'angle': 0})
        console.log('bb', JSON.stringify(aa))
        ...
        if (boxModel && boxModel.nodeType == fengmap.FMNodeType.BOX) {
          ...
          storageModel = group.getOrCreateLayer('transport').getTransport(boxModel.fid) // 点击货物，货柜高亮
    ...
    loadScrollFloorCtrl () {
      ...
      let scrollFloorControl = new fengmap.FMToolControl(
        this.map,
        toolCtlOpt
      )
    },
    ...
    fidField () {
      return new Promise((resolve) => {
        this.map.openTransportJson('../../../static/data/fid1.json', function (data) {
    ...
    getPosition () {
      ...
      var timer = setInterval(() => {
        if (index > positionData.length - 1) {
          ...
          if (positionData[index].x == '12677292.696621861') { // 到达仓库B位置
